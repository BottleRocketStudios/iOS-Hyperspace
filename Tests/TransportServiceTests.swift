...
class TransportServiceTests: XCTestCase {
    ...
    // MARK: - Tests
    func test_TransportService_ProperlyInitializesWithSessionConfiguration() async {
        let config = URLSessionConfiguration.ephemeral
        let service = TransportService(sessionConfiguration: config)

        let configuration = await service.session.configuration
...
// MARK: - Helper
private extension TransportServiceTests {

    func executeTransportServiceUsingMockHTTPResponse(_ mockHTTPResponse: HTTP.Response?,
                                                      mockError: Error? = nil,
                                                      expectingResult expectedResult: TransportResult,
                                                      file: StaticString = #file,
                                                      line: UInt = #line) async {
        let mockSession = MockTransportSession(responseStatusCode: mockHTTPResponse?.code, responseData: mockHTTPResponse?.body, error: mockError)
        await executeTransportService(using: mockSession, expectingResult: expectedResult, file: file, line: line)
    }
