...
public protocol BackendServicing {
    ...
    var recoveryStrategies: [any RecoveryStrategy] { get }

    /// Handles any just-in-time modifications to requests before they are executed.
    var preparationStrategies: [any PreparationStrategy] { get }
    ...
    func execute<R>(request: Request<R>, delegate: (any TransportTaskDelegate)?) async throws -> R
}
...
public extension BackendServicing {
    ...
    var recoveryStrategies: [any RecoveryStrategy] { return [] }
    var preparationStrategies: [any PreparationStrategy] { return [] }
    ...
    func attemptToRecover<R>(from error: any Error, executing request: Request<R>) async throws -> R {
    ...
    ///   this completion is passed to the recovered `Request` instance. In the case of a failed recovery, the completion should be passed an error.
    @available(iOS 15.0, tvOS 15.0, macOS 12.0, watchOS 8.0, *)
    func attemptToRecover<R>(from error: any Error, executing request: Request<R>, delegate: (any TransportTaskDelegate)? = nil) async throws -> R {
        for strategy in recoveryStrategies {
            let recoveryDisposition = await strategy.attemptRecovery(from: error, executing: request)
