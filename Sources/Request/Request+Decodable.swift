...
public extension Request where Response: Decodable {
    ...
    static func successTransformer(for decoder: JSONDecoder, errorTransformer: @escaping @Sendable (any Error) -> any Error = { $0 }) -> Transformer {
    ...
    static func successTransformer<C: DecodableContainer>(for decoder: JSONDecoder, with containerType: C.Type,
                                                          errorTransformer: @escaping @Sendable (any Error) -> any Error = { $0 }) -> Transformer where C.Contained == Response {
        return { transportSuccess in
            do {
